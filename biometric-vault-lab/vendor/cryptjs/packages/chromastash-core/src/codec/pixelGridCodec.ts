...
import type {
  ChromaMetadata, EncodeOptions, EncodeResult,
  DecodeOptions, DecodeResult, SlidePattern,
  ProgressCallback, Logger, EncryptionMethod,
} from '../types.js';
  ...
  for (let y = 0; y < effectiveRes; y++) {
    for (let x = 0; x < effectiveRes; x++) {
      if (hasMarkers) {
        ...
        if ((top && left) || (top && right) || (bot && left) || (bot && right)) continue;
      }
      coords.push({ x, y });
    }
  }
...
function calcTotalSlides(
  ...
  pixelBlockSize: number,
  meta: ChromaMetadata,
): { totalSlides: number; finalMetaBytes: Uint8Array } {
...
): { totalSlides: number; bytesPerSlide: number } {
...
function tryParseMetadata(bytes: number[], expectedPbs: number, log: Logger): ChromaMetadata | null {
  try {
    if (bytes.length < BYTES_PER_PIXEL) return null;
    const metaLen = (bytes[0] << 16) | (bytes[1] << 8) | bytes[2];
    if (metaLen <= 0 || metaLen > bytes.length - BYTES_PER_PIXEL) return null;

    const metaBytes = new Uint8Array(bytes.slice(BYTES_PER_PIXEL, BYTES_PER_PIXEL + metaLen));
    const metadata = JSON.parse(textDecoder.decode(metaBytes)) as ChromaMetadata;
    if (metadata.kind !== 'pixel-grid-meta') return null;
...
function readPixelBlockSize(img: ImageData, log: Logger): number {
...
export async function encode(
  ...
  if (isMoBrowser()) {
    const hasScrambling = (options.slidePatterns ?? []).some(
      (p) => p && p !== 'None',
    );
    if (!hasScrambling) {
      try {
  ...
  const {
    ...
    pixelBlockSize = DEFAULTS.PIXEL_BLOCK_SIZE,
    slidePatterns = [],
    cornerMarkers = DEFAULTS.CORNER_MARKERS,
    addBorder = DEFAULTS.ADD_BORDER,
    fileName = 'encoded-file',
  ...
  const meta = makeMetadata(
    fileName, mimeType, rawBytes.length, fileHash,
    resolution, pixelBlockSize, encryption, cornerMarkers,
  );
  meta.isScrambled = isScrambled;

  const { totalSlides, finalMetaBytes } = calcTotalSlides(
    dataToWrite.length, effectiveRes, cornerMarkers, pixelBlockSize, meta,
  );
  meta.totalSlides = totalSlides;
  ...
  const setPixelDirect = (
    imgData: ImageData, px: number, py: number, r: number, g: number, b: number, a = 255,
  ) => {
  ...
  const fillRectDirect = (
    imgData: ImageData, rx: number, ry: number, rw: number, rh: number,
  ...
  for (let s = 0; s < totalSlides; s++) {
    ...
    const drawPixel = (r: number, g: number, b: number) => {
      if (coordCursor >= dataCoords.length) return;
    ...
    if (s === 0) {
      ...
      drawPixel(
        (finalMetaBytes.length >> 16) & 0xff,
        (finalMetaBytes.length >> 8) & 0xff,
        finalMetaBytes.length & 0xff,
      );
      // Meta body
      let metaRgb = [0, 0, 0];
      let metaIdx = 0;
      ...
      if (metaIdx > 0) drawPixel(metaRgb[0], metaRgb[1], metaRgb[2]);
    }
    ...
    const pattern = (slidePatterns[s] ?? 'None') as SlidePattern;
    if (pattern !== 'None') {
...
export async function decode(
  slideBlobs: (Blob | File)[],
  options: DecodeOptions = {},
  onProgress: ProgressCallback = () => {},
  log: Logger = () => {},
): Promise<DecodeResult> {
  if (slideBlobs.length === 0) throw new Error('No slide images provided.');
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    if (corners) {
      ...
      if (alreadyAligned) {
        ...
        const isAxisAligned =
          Math.abs(corners.tl.y - corners.tr.y) < 3 &&
  ...
  const pattern0 = (slidePatterns[0] ?? 'None') as SlidePattern;
  if (pattern0 !== 'None') {
  ...
  let metadata: ChromaMetadata | null = null;
  ...
  if (!metadata) {
    throw new Error(
      'Could not read metadata from slide 0. Check patterns, block size, and slide order.',
    );
  }
  ...
  for (let s = 0; s < slidesImageData.length; s++) {
    ...
    const pat = (slidePatterns[s] ?? 'None') as SlidePattern;
    if (pat !== 'None') {
    ...
    for (let i = 0; i < slideBytes.length; i++) allBytes.push(slideBytes[i]);
  }
  ...
  if (effectiveEncryption !== 'none' && !secretKey) {
    throw new Error(
      `Decryption method '${effectiveEncryption}' requires a secret key.`,
    );
  }
  ...
  const decodedHash = await sha256(trimmed.buffer as ArrayBuffer);
