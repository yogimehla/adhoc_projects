...
function horizontalFlip(img: ImageData): ImageData {
...
function verticalFlip(img: ImageData): ImageData {
  const { width, height, data } = img;
  const out = new Uint8ClampedArray(data.length);
  for (let x = 0; x < width; x++) {
    for (let y = 0; y < height; y++) {
...
function rotate90CW(img: ImageData): ImageData {
  const { width, height, data } = img;
  const out = new Uint8ClampedArray(data.length);
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
...
function rotate90CCW(img: ImageData): ImageData {
  const { width, height, data } = img;
  const out = new Uint8ClampedArray(data.length);
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
...
const scrambleOps: Record<string, (img: ImageData) => ImageData> = {
  Invert: invert,
...
const unscrambleOps: Record<string, (img: ImageData) => ImageData> = {
  Invert: invert,
  'Horizontal Flip': horizontalFlip,
  'Vertical Flip': verticalFlip,
...
export function scramble(img: ImageData, pattern: SlidePattern): ImageData {
  if (!pattern || pattern === 'None') return img;
  const op = scrambleOps[pattern];
  return op ? op(img) : img;
}

/** Reverse a scrambling pattern on slide image data. */
export function unscramble(img: ImageData, pattern: SlidePattern): ImageData {
  if (!pattern || pattern === 'None') return img;
