 ...
 */
export function detectCorners(imageData: ImageData): CornerPoints | null {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      const i = y * width + x;
      if (binary[i] !== 1 || visited[i]) continue;

      const blob: Blob = { pixels: [], minX: x, minY: y, maxX: x, maxY: y };
      const queue = [{ x, y }];
      visited[i] = 1;
  ...
  const cx = width / 2;
  const cy = height / 2;
  const quads: Record<string, Blob[]> = { tl: [], tr: [], bl: [], br: [] };

  for (const c of candidates) {
    const mx = (c.minX + c.maxX) / 2;
    const my = (c.minY + c.maxY) / 2;
    if (mx < cx && my < cy) quads.tl.push(c);
    else if (mx > cx && my < cy) quads.tr.push(c);
    else if (mx < cx && my > cy) quads.bl.push(c);
    else quads.br.push(c);
  }
  ...
  const centre = (b: Blob) => ({ x: (b.minX + b.maxX) / 2, y: (b.minY + b.maxY) / 2 });
...
function solveHomography(
  ...
  p7: { x: number; y: number }, p8: { x: number; y: number },
): number[] {
