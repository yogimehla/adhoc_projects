...
function read32(arr: Uint8Array, offset: number): number {
  return (
...
// ═══════════════════════════════════════════════════════════════════════════
//  DECODE: PNG Blob → RGBA pixels
// ═══════════════════════════════════════════════════════════════════════════

export interface RawImageData {
  width: number;
  height: number;
  data: Uint8Array; // flat RGBA
}

/**
 * Decode a PNG Blob into raw RGBA pixel data without using canvas.
 */
export async function pngBlobToRgba(blob: Blob): Promise<RawImageData> {
  const buf = new Uint8Array(await blob.arrayBuffer());

  // Verify PNG signature
  for (let i = 0; i < 8; i++) {
    if (buf[i] !== PNG_SIGNATURE[i]) throw new Error('Not a valid PNG file');
  }
  ...
  let pos = 0;
  for (const c of idatChunks) { compressedData.set(c, pos); pos += c.length; }
