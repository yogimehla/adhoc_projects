...
function installResultHandler() {
  ...
  window._externalMessageResult = (result: any) => {
...
function sendIpc(payload: object): Promise<any> {
...
async function blobOrBufferToBase64(data: Blob | ArrayBuffer): Promise<string> {
...
export async function ipcEncode(
  ...
  const slides: Blob[] = response.slides.map((b64: string) =>
    base64ToBlob(b64, 'image/png'),
  );
  const metadata: ChromaMetadata = response.metadata;
...
export async function ipcDecode(
  slides: (Blob | File)[],
